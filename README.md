...
## Recent Highlights/Additions
    ...
    - Validated synthetic turbulence generator for supply vents
    - Validated quadrature method of moments approach for eulerian particle tracking
    - Airborne disease infection risk model
    - IBM translational movements, heat sources, and concentration sources

## Executables

    - tosca    : transient solver for incompressible flows. Temperature stratification is accounted via Boussinesq approximation.
...
## Boundary Conditions
    ...
    - slip                 : on all patches
    - zeroGradient         : on all patches (deposition wall model overrides this for moment BC if deposition is on)
    - fixedGradient        : on all patches (only T equation)
    - inflowFunction       : inflow function 7 is used for synthetic turbulence. Adds turbuelnce to a 1/7th power rule profile.
    
    #vents: to run vented boundary conditions, set the main BC to noSlip and add vents where desired.
    - zeroGradient
    - fixedValue 
    - inflowFunction 7     : synthetic turbulence
