...
We acknowledge that there is not much information around that explains how TOSCA works and how to setup TOSCA cases, but we
are committed to change that in the future. A temporary user guide with an explanation of all input parameters and boundary conditions can be found inside `docs/` (*TOSCA_UserGuide.pdf*). For programmers, the code follows the Doxygen commenting style, so that a programmer's documentation is automatically generated by Doxygen. This can be found both inside `docs/` (*ReferenceManual.pdf*), or opened as a web documentation upon downloading the TOSCA repository and opening `docs/html/index.html`.

At the moment, we are working on an extended user guide through Sphinx, using the Read The Docs template, which seems to us the solution most people are more familiar with. This guide is still under construction and will updated in the future. To view its latest release one can open it as a web documentation upon downloading the TOSCA repository and opening `docs/sphinx/build/html/index.html`. This has been also deployed on our GitHub page, but this is still a work in progress.
