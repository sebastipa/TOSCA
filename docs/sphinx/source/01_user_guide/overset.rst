...
are blanked when they overlap with the finer mesh. This "hole" is the background mesh is treated as an IBM body (see :ref:`ibm-section`), 
...
goes from parent to child (coarse to file), a tri-linear scheme is used. Conversely, when going from child to parent (fine to coarse), a tri-linear 
scheme would make the interpolated value too local if the parent to child grid ration is too large. For this reason, TOSCA introduces a 
...
be placed inside the *boundary/meshName* sub-directory, as they are domain-specific. For child domains, at the boundary where one wants to 
