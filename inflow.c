    ...
    if(mesh->boundaryU.kLeft == "inletFunction")
    {
        ...
        else if (ifPtr->typeU == 3)
        {
            ...
            if(ifPtr->merge1)
            {
                ...
                // into the global uBarAvgTopX / tBarAvgTopX on every rank. This replaces the
                // previous "every rank reads every file" pattern whose filesystem-metadata
                // contention dominated wall time at high rank counts.
        ...
        else if (ifPtr->typeU == 4)
        {
            ...
            if(ifPtr->merge1)
            {
                ...
                // uBarAvgTopX is 10 Cmpnts = 30 PetscReals; tBarAvgTopX is 10 PetscReals.
                MPI_Allreduce(MPI_IN_PLACE, (PetscReal*)&(ifPtr->uBarAvgTopX[0]), 30, MPIU_REAL, MPI_SUM, mesh->MESH_COMM);
                if(mesh->access->flags->isTeqnActive)
                {
                    MPI_Allreduce(MPI_IN_PLACE, &(ifPtr->tBarAvgTopX[0]), 10, MPIU_REAL, MPI_SUM, mesh->MESH_COMM);
...
PetscErrorCode SetInflowWeights(mesh_ *mesh, inletFunctionTypes *ifPtr)
{
    ...
    //
    // Pack owner-rank contributions into flat contiguous buffers, do ONE MPI_Allreduce per buffer,
    // then unpack. Replaces (my-2)*(mx-2)*(2 or 6) tiny per-cell Allreduces that dominate wall time
    // at high rank counts because each collective carries a fixed latency that grows with rank count.
    {
        ...
        // ----- spline path (only if interpMethod == "spline") -----
        if (ifPtr->interpMethod == "spline")
        {
