...
This is done to avoid reading the reference state from file, which would be unfeasible both from a I/O and storage perspective.

The concurrent precursor can be both autosustained by using periodic BCs on all lateral boundaries, or fed with an *inflowDatabase*. In the first case,
a spin-up phase is required to develop the turbulent ABL, while in the second case one only needs to wait one flow-turnover time for the precursor domain 
...
condition to drive the flow in the concurrent precursor. 
...
When the simulation includes potential temperature, gravity waves can develop in the domain. These waves then reflect on the boundaries, polluting the flow 
solution. It has been shown in literature that fringe and Rayleigh damping regions applied at the domain boundaries help avoid this effect, if correctly designed.
Without concurrent precursor method, one would require three fringe regions, namely at the inlet, outlet and domain top. Moreover, it becomes difficult to 
apply the inflow and outflow reference states inside the ABL, as they are turbulent and thus time-varying. For this reason, finite-volume codes usually use 
Rayleigh damping regions at all boundaries, where the inlet and outlet regions do not extend inside the boundary layer. This might work in some cases, but it is 
...
works as a hybrid Rayleigh-fringe region depending if source terms are only applied in the vertical direction (steady) or also in the horizontal (unsteady). 
...
specify the mesh for the precursor domain, as this is internally done in TOSCA, including the handling of the domain decomposition. The phylosophy 
under which this is achieve is the following: 

1. The processors that are (fully or partially) contained in the fringe region are selected. 
2. The cells that belong to these processors are selected and will be used to create the new mesh.

This means that the precursor domain is only solved by a subset of the processors that are used for the wind farm simulation. The remaining processors 
will idle while the precursor time step is computed. If the time step is adjustable based on the CFL condition, the maximum velocitu between the two 
...
is run with the optimal cell-per-processor ratio, the same ratio is employed for the precursor simulation.
...
equal to 1 or 2 in the *control.dat* file. Notably, **they are not required** if ``-precursorSpinUp`` is set t 0. The ``n1Inflow`` and ``n2Inflow`` entries define the 
...
the inflow data is averaged at the top 10 cells, to make the inflow truly steady above. In this case, the inflow is periodized 2 times in the y direction and 1 time 
in the x direction. Above the available data, the precursor inlet patch is padded with the last value that is found in the inflow database. For more information on 
...
remove the streamwise turbulent streaks that sometimes produce a slow convergene of the statistics. 
...
.. note::
    ...
    Notably, wrong settings may lead to a non-physical solution, or worse case, to a crash of the simulation. The settings provieded in the current example are just 
    explanatory and should be adapted to the specific case. Thorough information on how to set these regions can be found `here <https://wes.copernicus.org/articles/9/297/2024/#section9>`_ 
...
and, starting from the 25th processor, the *concurrent solution flag* is set to 0, as these processors do not own any fringe region cell. After the concurrent 
precursor mesh creation case, the mesh file is written in the case directory, so that precursor fields can be later processed using the utility ``tosca2PV``. 
the user has to make sure that the first lines of the wind farm mesh file (specifying the periodicty, if any) are correctly copied to the precursor mesh file.
...
The following figure shows the simulation setup, with the reciprocal position of the fringe region (blue), the Rayleigh damping region (red) and the advection damping 
region (green). The wind farm domain is shown with opacity, together with a horizontal section through the hub-height of the wind turbibes. 
A concurrent precursor section, at the same height, is alwso shown in from of the wind farm domain for clarity, but in reality it is contained in the blue region, as 
...
As can be seen, the wind the wind turbines wakes that are leaving the domain are re-advected at the inlet by the periodic boundary conditions, but are then eliminated 
