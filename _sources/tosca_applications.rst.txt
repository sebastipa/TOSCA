...
introduce refined regions which are two-way coupled to main background domain. TOSCA features a dynamic 
...
A handful of data assimilation techniques has been recently introduced in TOSCA, which allow to nudge the atmospheric boundary layer 
...
Recently, TOSCA has also been coupled with the OpenFAST library, introducing blade flexibility. At this time, boundary coupling with the WRF
model is under construction, and there are ongoing efforts to port parts of the TOSCA code to GPUs, as well as including multiphase 
capabilities to model ABL-wave interaction and floating wind turbines.  

Currently, TOSCA can be used to simulate atmospheric boundary layers and wind farms over sea or complex terrain, including thermal 
